...
void single_fastq_to_fastq(char *fq1_fn, char *fq_out, const read_s read_structure, const filter_s filter_settings)
{
    ...
    // naming states
    const int ONE_INDEX_WITH_UMI = 0;
    const int ONE_INDEX_NO_UMI = 1;

    // set barcode end index
    if (umi_st >= 0)
    {
        if (id1_st + id1_len > umi_st + umi_len)
        {
            bc1_end = id1_st + id1_len;
        }
        else
        {
            bc1_end = umi_st + umi_len;
        }
        state = ONE_INDEX_WITH_UMI;
    }
    else
    {
        state = ONE_INDEX_NO_UMI;
        bc1_end = id1_st + id1_len;
    }

    // set offset for fastq header
    int name_offset;
    if (state == ONE_INDEX_WITH_UMI)
    {
        name_offset = id1_len + umi_len + 2;
    }
    else if (state == ONE_INDEX_NO_UMI)
    {
        name_offset = id2_len + 2;
    ...
    while (((l1 = kseq_read(seq1)) >= 0))
    {
        ...
        {
            memcpy(seq1->name.s, seq1->seq.s+id1_st, id1_len*sizeof(char)); // copy index one
            seq1->name.s[id1_len] = '_'; // add separator
            memcpy(seq1->name.s+id1_len+1, seq1->seq.s+umi_st, umi_len*sizeof(char)); // copy umi
        }
        else if (state == ONE_INDEX_NO_UMI)
        {
            memcpy(seq1->name.s, seq1->seq.s+id1_st, id1_len*sizeof(char)); // copy index two
            seq1->name.s[id1_len] = '_'; // add separator
        }
        seq1->name.s[name_offset-1] = '#';
        seq1->name.l = name_offset + seq1->name.l;
